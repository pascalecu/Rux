...
function(rux_build_std_database selected_std out_version out_macro out_avail)
    ...
    list(LENGTH RUX_CPP_STANDARDS _db_len)
    set(_i 0)

    while (_i LESS _db_len)
        list(GET RUX_CPP_STANDARDS ${_i} _std)
        math(EXPR _j "${_i} + 1")
        list(GET RUX_CPP_STANDARDS ${_j} _macro)

        # Export macro definition to parent scope
        set(RUX_CPP_STD_${_std}_MACRO ${_macro} PARENT_SCOPE)
        ...
        set(RUX_CPP_STD_${_std}_VERSION ${_temp_date} PARENT_SCOPE)
...
function(rux_check_feature_availability current_version)
    list(LENGTH RUX_CPP_KNOWN_STANDARDS _known_len)
    set(_k 0)

    while (_k LESS _known_len)
        list(GET RUX_CPP_KNOWN_STANDARDS ${_k} _std)

        if (current_version GREATER_EQUAL RUX_CPP_STD_${_std}_VERSION)
            set(RUX_LANG_STDCPP_AT_LEAST_${_std} 1 PARENT_SCOPE)
            message(STATUS "\tC++${_std}: YES")
        else ()
            set(RUX_LANG_STDCPP_AT_LEAST_${_std} 0 PARENT_SCOPE)
...
set(RUX_LANG_STDCPP 0)
set(RUX_LANG_STDCPP_AVAILABLE 0)
...
set(RUX_CPP_KNOWN_STANDARDS 98 11 14 17 20 23 26)
...
message(STATUS "\tSelected : C++${RUX_CPP_STANDARD}")

set(CMAKE_CXX_STANDARD ${RUX_CPP_STANDARD})
...
rux_build_std_database(
        ${RUX_CPP_STANDARD}
        RUX_LANG_STDCPP
        RUX_LANG_STDCPP_MACRO
        RUX_LANG_STDCPP_AVAILABLE
)

if (NOT RUX_LANG_STDCPP_AVAILABLE)
    message(FATAL_ERROR "Failed to resolve active C++ standard")
endif ()

# Feature availability
message(STATUS "C++ standard is at least:")
rux_check_feature_availability(${RUX_LANG_STDCPP})

# Exported values
set(RUX_LANG_STDCPP_VERSION ${RUX_LANG_STDCPP})

message(STATUS "Final summary:")
message(STATUS "\tStandard   : C++${RUX_CPP_STANDARD}")
message(STATUS "\t__cplusplus: ${RUX_LANG_STDCPP_MACRO}")
message(STATUS "\tVersion    : ${RUX_LANG_STDCPP}")
