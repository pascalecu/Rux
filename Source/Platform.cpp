...
[[nodiscard]] RuntimeCpuInfo const &CachedCpuInfo() noexcept {
    static RuntimeCpuInfo const info = DetectRuntimeCpuInfo();
    return info;
}
...
MemoryInfo GetRuntimeMemoryInfo() noexcept {
    ...
    if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vm, &c) ==
        KERN_SUCCESS) {
