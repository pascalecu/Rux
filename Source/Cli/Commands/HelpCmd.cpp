...
constexpr std::size_t MaxSize(Range const &range, Proj proj = {}) {
    std::size_t maxSize = 0;

    for (auto const &item : range) {
        auto const &value = std::invoke(proj, item);
...
std::size_t GetTerminalWidth() {
    ...
    if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi) == TRUE) {
        auto const width = csbi.srWindow.Right - csbi.srWindow.Left + 1;
...
constexpr std::size_t UsableWidth(std::size_t const terminalWidth, std::size_t const indent) {
...
constexpr void ProcessLine(std::string_view line, std::size_t const width, Callback callback) {
    while (!line.empty()) {
        // Skip leading spaces by sliding the view forward
        auto const firstNonSpace = line.find_first_not_of(Layout::Whitespace);
        ...
        auto const lastNonSpace = piece.find_last_not_of(Layout::Whitespace);
...
template <typename Callback>
constexpr void Wrap(std::string_view text, std::size_t const width, Callback callback) {
    while (!text.empty()) {
        auto const newLinePos = text.find('\n');

        // substr handles npos gracefully when anchored at 0
        if (std::string_view const line = text.substr(0, newLinePos); line.empty()) {
            callback(""sv);
        }
        else {
...
void PrintBlock(std::string_view title, std::string_view const text, size_t const termWidth,
                size_t const indent = Layout::BlockIndent) {
    if (text.empty()) {
        return;
    }
    ...
    size_t const usable = UsableWidth(termWidth, indent);
...
void PrintAligned(std::string_view const left, std::string_view const right, size_t const leftWidth,
                  size_t const termWidth) {
    size_t const indent = Layout::BlockIndent + leftWidth + Layout::AlignedPadding;
    size_t const width = UsableWidth(termWidth, indent);
    bool first = true;

    Wrap(right, width, [&](std::string_view line) {
        if (first) {
            std::println("{0:<{1}}{2:<{3}}{4}", "", Layout::BlockIndent, left,
                         leftWidth + Layout::AlignedPadding, line);
...
namespace Data {
// Add
constexpr std::array add_usage = {"[package]"sv, "[package]@[version]"sv,
...
constexpr std::array build_opts = {
    ...
    OptionDoc{.flags = "--profile <n>"sv,
              .desc = "Build using a custom profile defined in Rux.toml"sv},
    OptionDoc{.flags = "--release"sv,
              .desc = "Build with release profile (optimized, no debug info)"sv},
    ...
    OptionDoc{.flags = "-q, --quiet"sv,
              .desc = "Suppress non-essential output (only errors are shown)"sv},
    OptionDoc{.flags = "-v, --verbose"sv,
              .desc = "Enable verbose output for detailed build information"sv},
    ...
    OptionDoc{.flags = "--dump-sema"sv,
              .desc = "Write semantic analysis results to Temp/Sema/sema.txt"sv},
    OptionDoc{.flags = "--dump-tokens"sv,
              .desc = "Write the token stream to Temp/Tokens/<file>.tokens"sv}};
...
constexpr std::array doc_opts = {OptionDoc{
    .flags = "--open"sv, .desc = "Open the generated documentation index in a browser"sv}};
...
constexpr std::array fmt_opts = {
    ...
    OptionDoc{.flags = "--manifest"sv,
              .desc = "Format only the manifest configuration file (Rux.toml)"sv}};
...
// Install
constexpr std::array install_usage = {""sv, "[package]"sv, "[package]@[version]"sv,
                                      "--dev [package]"sv};
...
constexpr std::array new_opts = {
    OptionDoc{.flags = "--bin"sv,
              .desc = "Create an executable binary package application (default)"sv},
...
constexpr std::array update_opts = {OptionDoc{
    .flags = "--global"sv, .desc = "Update all entries in the global environment cache"sv}};
...
constexpr auto VerifyRegistryIntegrity() -> bool {
    if constexpr (constexpr auto dup =
                      std::ranges::adjacent_find(G_COMMAND_HELP_MAPS, {}, &CommandDoc::name);
...
void Cli::PrintHelpFor(std::string_view const command) {
    if (command == "help") {
        PrintHelp();
        return;
    }
    ...
    auto const &[name, shortDesc, description, usage, postUsage, footer, examples, options] = *it;
    size_t const termWidth = GetTerminalWidth();

    std::string_view const longOrShortDescription = !description.empty() ? description : shortDesc;
    Wrap(longOrShortDescription, termWidth,
         [&](std::string_view line) -> void { std::println("{}", line); });
    std::println("");
    ...
    if (usage.empty()) {
        ...
        for (std::string_view const usageVariant : usage) {
    ...
    if (!options.empty()) {
        auto const optWidth = MaxSize(options, &OptionDoc::flags);

        std::println("Options:");
        for (auto const &[flags, desc] : options) {
...
void Cli::PrintUnknownOption(std::string_view option, std::string_view command) {
    if (command.empty()) {
        ...
        std::println(stderr, "\nUse 'rux help {}' for more information about this command.",
                     command);
    }
}
